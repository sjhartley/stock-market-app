      ...
      constituents('table#constituents tbody tr').each(function(i, el){
        ...
        if((locationObj.name !== null) && (locationObj.location !== null) && (locationObj.url !== null)){
          ...
          if(idx == -1){
            ...
            compObj.gics_sector=gics_sector.text();
            compObj.gics_sub=gics_sub.text();
            no_dupl_url[idx].compObj.push(compObj);
...
showStateMarkers(state){
  self=this;
    console.log(state);
    idx=self.state.states.findIndex(e => e.state == state);
    console.log(`idx=${idx}`);
    if(idx !== -1){
      console.log(this.state.states[idx].locations);
      this.setState({ markers: this.state.states[idx].locations});
      region={
        latitude: this.state.states[idx].locations[0].latitude,
        longitude: this.state.states[idx].locations[0].longitude
      };
      console.log(region);
      delta_arr=this.delta_calc(this.state.states[idx].locations);
...
loadMarker = (loadMarkers_enable) =>{
  ...
  if(loadMarkers_enable == 0){
    ...
    return(
      ...
      <View style={{paddingLeft: 100, paddingRight: 100, paddingTop: 50}}>
      <TouchableOpacity style={{backgroundColor: '#FF0000'}} onPress={() => this.showStateMarkers(this.state.selectedState)} >
      ...
      <MapView
        ...
        this.state.markers.map((entry, index) =>{
            ...
            return(
              ...
              onPress={() => this.modal_show(entry.name)}/>
            )
            }
        })
